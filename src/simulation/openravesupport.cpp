...
void RaveRobotObject::initRobot(const btTransform &initialTransform, TrimeshMode trimeshMode, float fmargin, bool isDynamic) {
    ...
    for (std::vector<KinBody::LinkPtr>::const_iterator link = links.begin(); link != links.end(); ++link) {
        //const std::vector<KinBody::Link::GEOMPROPERTIES> geometries = (*link)->GetGeometries();
        std::vector<boost::shared_ptr<KinBody::Link::Geometry> > geometries = (*link)->GetGeometries();
        ...
        for (std::vector<boost::shared_ptr<KinBody::Link::Geometry> >::const_iterator geom = geometries.begin();
            ...
            case KinBody::Link::Geometry::GeomTrimesh:
